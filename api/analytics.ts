...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      weekSummary: {
        total_submissions: recentWeekAnalytics.reduce((sum: number, day: AnalyticsDay) => sum + (day.total_submissions || 0), 0 as number),
        unique_visitors: recentWeekAnalytics.reduce((sum: number, day: AnalyticsDay) => sum + (day.unique_visitors || 0), 0 as number),
        honeypot_catches: recentWeekAnalytics.reduce((sum: number, day: AnalyticsDay) => sum + (day.honeypot_catches || 0), 0 as number),
      },
