...
export function useErrorBoundary(config: ErrorHandlerConfig = {}) {
  ...
  const attemptRecovery = useCallback(async (recoveryStrategies: ErrorRecovery[]) => {
...
export function useNetworkErrorHandler(config: ErrorHandlerConfig = {}) {
  ...
  const handleNetworkError = useCallback((error: ErrorInfo) => {
...
export function useErrorRecovery() {
  ...
  const executeRecovery = useCallback(async (error: ErrorInfo): Promise<boolean> => {
...
export const errorUtils = {
   ...
   */
  createErrorBoundary: (Component: React.ComponentType<any>, fallback?: React.ComponentType<any>) => {
