...
export function useRenderMonitor(componentName: string) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      logger.info(
        `${componentName} unmounted after ${count} renders. Avg render time: ${avgRenderTime.toFixed(2)}ms`
      );
    };
  });
  ...
  const getStats = useCallback(() => {
    ...
    return {
      ...
      minRenderTime: count > 0 ? (minRenderTime.current === Infinity ? 0 : minRenderTime.current) : 0,
      renderTimes: [], // Return empty array to avoid breaking the interface while saving memory
    };
  }, []);
  return {
    getStats
