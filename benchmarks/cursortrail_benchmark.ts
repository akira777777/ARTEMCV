...
function runCurrent() {
  const trail: TrailPoint[] = Array(TRAIL_LENGTH)
    .fill(null)
    .map(() => ({
      x: 0,
      y: 0,
...
function runOptimized() {
  ...
  const hueStrings = Array(TRAIL_LENGTH)
    .fill(0)
    .map((_, i) => 160 + (i / TRAIL_LENGTH) * 100);
...
console.log(
  `Improvement: ${(currentDur - optimizedDur).toFixed(2)}ms (${(((currentDur - optimizedDur) / currentDur) * 100).toFixed(2)}%)`,
);
