...
export const I18nProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [lang, setLangState] = useState<Lang>('en'); // Default to 'en' to match server render
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    const fromStorage = localStorage.getItem('lang') as Lang | null;
    if (fromStorage && ['en', 'ru', 'cs'].includes(fromStorage)) {
      setLangState(fromStorage);
    } else {
      const nav = navigator.language.toLowerCase();
      if (nav.startsWith('cs')) setLangState('cs');
  ...
  // or accept that initial render might be English.
  // For a portfolio, consistent initial render (en) is better than flicker.
  // But if we want to show correct lang immediately, we need to suppress hydration warning or wait.
  // We'll return context provider always, but lang will update client-side.
