    ...
    for (const lang of LANGUAGES) {
      test(`Should display correct content in ${lang.toUpperCase()}`, async ({ page }) => {
        ...
        const heroBadge = page
          .locator(
            'text=Senior Frontend Architect, text=Старший фронтенд-архитектор, text=Senior Frontend Architekt',
          )
          .first();
  ...
  test.describe('5. Visual Components Testing', () => {
    ...
    test('Services section should display correctly', async ({ page }) => {
      ...
      const serviceCards = page
        .locator('[class*="service"]')
        .or(page.getByText(/Web Development|UI\/UX Design/i));
    ...
    test('Work gallery should function properly', async ({ page }) => {
      ...
      const projectCards = page
        .locator('[class*="project"]')
        .or(page.getByText(/Detailing Hub|Dental Clinic|Barbershop/i));
